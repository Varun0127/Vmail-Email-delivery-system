...
<%
   try {
        ...
        PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM Emails WHERE user_id = ? and folder='sent' order by updated_at desc");
                  						...
                  						<div class="table-responsive">
                                        	<table id="inboxTable" class="table table-hover">
                                            	...
                                            	<tbody>
                                            		...
                                            		%>
                                            		<tr onclick="if(event.target.tagName !== 'I' && event.target.tagName !== 'INPUT')window.location='emailDetails.jsp?id=<%=email.getEmailId()%>&status=<%=email.getStatus() %>&folder=<%=email.getFolder()%>';" style="cursor:pointer;">
                                                    	<td><input type="checkbox" class="mailCheckbox" name="checkboxitem" value="<%= email.getEmailId() %>"></td>
                                                        <td><%= ((email.getRecipientId().length())>25)?"To: "+(email.getRecipientId().substring(0,25)+"..."):"To: "+email.getRecipientId()%></td>
