...
<%
   try {
        ...
        PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM Emails WHERE user_id = ? and folder='inbox' order by updated_at desc");
                                        	...
                                        	<table id="inboxTable" class="table table-hover">
                                            	...
                                            	<tbody>
                                            		...
                                            		<tr onclick="if(event.target.tagName !== 'I' && event.target.tagName !== 'INPUT')window.location='emailDetails.jsp?id=<%=email.getEmailId()%>&status=<%=email.getStatus() %>&folder=<%=email.getFolder()%>';" style="cursor:pointer;" <% if("unread".equals(email.getStatus())) { %>class="font-weight-bold"<% } %> >
                                                        ...
                                                        <td>
                                                        	<a href="moveToBin.jsp?id=<%=email.getEmailId()%>&folder=<%=email.getFolder() %>" title="Delete"><i class="mdi mdi-delete deleteEmail"></i></a>
                                            		...
                                            		%>
                                                    <tr>
                                                    	<td colspan="5">No emails in inbox</td>
                                                    </tr>
                                            		<%
                                                	}
                                            		%>
