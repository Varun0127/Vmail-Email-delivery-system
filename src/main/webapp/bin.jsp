...
<%
   try {
        ...
        PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM Emails WHERE folder='bin' and user_id=? order by updated_at desc");
                  						...
                  						<div class="table-responsive">
                                        	<table id="inboxTable" class="table table-hover">
                                            	...
                                            	<tbody>
                                            		...
                                            		%>
                                            		<tr onclick="if(event.target.tagName !== 'I' && event.target.tagName !== 'INPUT')window.location='emailDetails.jsp?id=<%=email.getEmailId()%>&status=<%=email.getStatus() %>&folder=<%=email.getFolder()%>';" style="cursor:pointer;" <% if("unread".equals(email.getStatus())) { %>class="font-weight-bold"<% } %>>
                                                    	<td><input type="checkbox" class="mailCheckbox" name="checkboxitem" value="<%= email.getEmailId() %>"></td>
                                                        <td><%= ((email.getSenderId().length())>50)?(email.getSenderId().substring(0,25)+"..."):email.getSenderId()%></td>
                                                        ...
                                                        <td>
                                                        	<a href="deleteMail.jsp?id=<%=email.getEmailId()%>" title="Delete forever"><i class="mdi mdi-delete deleteEmail"></i></a>
                                                            <% if("unread".equals(email.getStatus())) { %>
                                                            	&emsp;<a href="mark_Read_Unread.jsp?id=<%=email.getEmailId()%>&status=<%=email.getStatus()%>&folder=<%=email.getFolder() %>" title="Mark as read"><i class="mdi mdi-email-open"></i></a>
                                                            <% } 
                                                            else{ %>
                                                            	&emsp;<a href="mark_Read_Unread.jsp?id=<%=email.getEmailId()%>&status=<%=email.getStatus() %>&folder=<%=email.getFolder() %>" title="Mark as unread"><i class="mdi mdi-email-mark-as-unread markUnread"></i></a>
                                                           <% 
                                                           	}                                                            
                                                            %>
                                                            &emsp;<a href="restoreMail.jsp?id=<%=email.getEmailId()%>" title="Restore mail"><i class="mdi mdi-file-restore" title="Restore mail"></i></a>
                                            		...
                                            		%>
                                                    <tr>
                                                    	<td colspan="5">No emails in bin</td>
                                                    </tr>
                                            		<%
                                                	}
                                            		%>
